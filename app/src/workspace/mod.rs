...
fn add_overflow_menu_items_as_editable_binding(app: &mut AppContext) {
    ...
    //
    // Terminal-only build: the entries mirroring removed avatar-menu items
    // ("Invite People...", "Join our Slack community", "Send feedback") are gone
    // too, so the palette cannot reach what the menu no longer offers.
    app.register_editable_bindings([
        EditableBinding::new(
